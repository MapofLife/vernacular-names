...
open(my $fh, "<:encoding(utf8)", "data/new_world_palms_2014may2_with_higher.csv")
...
while(my $row = $csv->getline_hr($fh)) {
    ...
    try {
        ...
        try {
            my $taxonHierarchy = decode_json($row->{'eol_hierarchy_entries'});
            ...
            warn "Could not parse taxon hierarchy: <<" . $row->{'eol_hierarchy_entries'} . ">>";
        };
