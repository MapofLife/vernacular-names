...
open(my $fh, "<:encoding(utf8)", "data/missing_amphibians_2014jul8.csv")
...
while(my $row = $csv->getline_hr($fh)) {
    ...
    my $source = "EOL API calls, July 8, 2014: amphibians missing from the previous run";
    ...
    try {
        ...
        try {
            my $taxonHierarchy = decode_json($row->{'eol_higher'});
            ...
            warn "Could not parse taxon hierarchy: <<" . $row->{'eol_higher'} . ">>";
        };
